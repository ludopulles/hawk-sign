 ...
 */
TARGET_AVX2
void Zf(poly_inner_prod)(fpr *restrict d,
	...
	const fpr *restrict f, const fpr *restrict g, unsigned logn)
{
	...
	for (u = 0; u < hn; u += 4) {
		...
		a_re = FMSUB(F_re, f_re, _mm256_mul_pd(F_im, f_im));
		a_im = FMADD(F_im, f_re, _mm256_mul_pd(F_re, f_im));
		b_re = FMSUB(G_re, g_re, _mm256_mul_pd(G_im, g_im));
		b_im = FMADD(G_im, g_re, _mm256_mul_pd(G_re, g_im));
...
static void
calc_target_NTT(int8_t *x0, int8_t *x1, int8_t *h0, int8_t *h1,
	int8_t *f, int8_t *g, int8_t *F, int8_t *G)
{
	...
	for (u = 0; u < n; u ++) {
		t0[u] = (uint16_t)h0[u] & 1;
		t1[u] = (uint16_t)h1[u] & 1;
		t2[u] = (uint16_t)f[u] & 1;
		t3[u] = (uint16_t)g[u] & 1;
		t4[u] = (uint16_t)F[u] & 1;
		t5[u] = (uint16_t)G[u] & 1;
	...
	for (u = 0; u < n; u ++) {
		// x0 = f h0 + F h1
		t6[u] = mq_add(mq_montymul(t2[u], t0[u]), mq_montymul(t4[u], t1[u]));
		// x1 = g h0 + G h1
		t7[u] = mq_add(mq_montymul(t3[u], t0[u]), mq_montymul(t5[u], t1[u]));
	}
	mq_iNTT(t6, logn);
	mq_iNTT(t7, logn);
	...
	for (u = 0; u < n; u ++) {
		x0[u] = (t6[u] ^ (uint16_t)((Q >> 1) - t6[u]) >> 15) & 1;
		x1[u] = (t7[u] ^ (uint16_t)((Q >> 1) - t7[u]) >> 15) & 1;
	}
}

static void
calc_target_FFT(int8_t *x0, int8_t *x1, int8_t *h0, int8_t *h1,
	int8_t *f, int8_t *g, int8_t *F, int8_t *G)
{
	size_t u;
	fpr *t0, *t1, *t2, *t3, *t4, *t5, *t6, *t7;
	...
	for (u = 0; u < n; u++) {
		x0[u] = fpr_rint(t6[u]) & 1;
		x1[u] = fpr_rint(t7[u]) & 1;
	}
}

static void
calc_target_bitset(int8_t *x0, int8_t *x1, int8_t *h0, int8_t *h1,
	int8_t *f, int8_t *g, int8_t *F, int8_t *G)
{
	size_t u;
...
int main() {
	...
	unsigned _seed = 1000000 * tv.tv_sec + tv.tv_usec;
	printf("Seed: %u\n", _seed);
	srand(_seed);
	...
	// expected:
	int8_t expx0[n], expx1[n];
	for (size_t u = 0; u < n; u++)
		expx0[u] = expx1[u] = 0;

	for (size_t u = 0; u < n; u++) {
		for (size_t v = 0; v < n; v++) {
			expx0[(u+v)&(n-1)] ^= ((h0[u] & f[v]) ^ (h1[u] & F[v])) & 1;
			expx1[(u+v)&(n-1)] ^= ((h0[u] & g[v]) ^ (h1[u] & G[v])) & 1;
		}
	}

	calc_target_NTT(x0, x1, h0, h1, f, g, F, G);
	for (size_t u = 0; u < n; u++) assert(x0[u] == expx0[u] && x1[u] == expx1[u]);
	calc_target_FFT(x0, x1, h0, h1, f, g, F, G);
	for (size_t u = 0; u < n; u++) assert(x0[u] == expx0[u] && x1[u] == expx1[u]);
	calc_target_bitset(x0, x1, h0, h1, f, g, F, G);
	for (size_t u = 0; u < n; u++) assert(x0[u] == expx0[u] && x1[u] == expx1[u]);
